...
product_category = Table(
    'product_category',
    Base.metadata,
    Column('product_id_pk', Integer, ForeignKey('product.product_id_pk'), primary_key=True),
    Column('category_id', Integer, ForeignKey('category_hierarchy.category_id'), primary_key=True)
)
...
class Product(Base):
    ...
    categories = relationship("CategoryHierarchy", secondary=product_category, back_populates="products")
...
class ProductPricing(Base):
    ...
    product_id_pk = Column(Integer, ForeignKey("product.product_id_pk"), primary_key=True)
